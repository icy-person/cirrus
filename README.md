...
| 🤔 **Reasoning traces** | `thinking` deltas stream into a collapsible section, with effort control for models that support it. Collapsed while it is being written — a trace is usually longer than the answer it precedes, and rendering one that grows by a token every few milliseconds costs a layout pass over the whole of it each time. Open it and you get the opening — where a model states the problem and picks an approach, which is the part that explains the answer — with all of it once it finishes. |
...
| 📤 **The model can hand you a file** | It writes into a private scratch space, so a path in there was never something you could act on — `save_file` copies what it made into your Downloads and tells you the name it landed under. |
| 🗂️ **A file browser for the scratchpad** | Everything the shell has written in a conversation, grouped by the job it belongs to, with a viewer that knows the difference between formats: markdown as prose, CSV as a table with its quoted fields intact, JSON and HTML and source highlighted, images decoded, and an honest "nothing to show" for anything binary. A download button on every file puts it in your Downloads. Before this the only way to see your own working files was to ask the model to print them back at you one `cat` at a time. |
