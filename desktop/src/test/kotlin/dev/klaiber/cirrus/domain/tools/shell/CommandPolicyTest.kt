...
class CommandPolicyTest {
    ...
    /**
     * `git` is the one program the policy has an opinion about in two places, and they must not
     * collide.
     *
     * The subcommand rule is the authority on it — read-only faces through, everything that
     * changes a repository or touches the network refused — so `git` stays out of the toolchain map
     * even though every other version control system is in it. Sweeping it in would silently
     * demote that rule to dead code and change the answer for `git log` as a side effect of a
     * change aimed at `git init`. (Today it is refused either way: `git` is not on the allow list
     * at all, which is why both calls below refuse.)
     */
    @Test
    fun `git is left to the subcommand rule rather than the toolchain map`() {
        refused("git log --oneline")
        refused("git init")
        assertTrue(!CommandPolicy.toolchainPrograms.containsKey("git"))
    }
